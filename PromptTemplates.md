...
11. Treat local evaluation as a supporting signal only. Do not optimize against local evaluation alone.
12. After at most 2 local evaluation runs for the current hypothesis, call `{eval_tool_name}` before continuing with further optimization work.
13. If you complete a candidate patch that you would otherwise continue iterating on, call `{eval_tool_name}` before making another substantial round of changes.
14. Do not go long stretches of local-only iteration when `{eval_tool_name}` budget remains.
